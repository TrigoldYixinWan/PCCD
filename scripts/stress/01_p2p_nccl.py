...
def _nccl_worker(rank, world):
    """Module-level worker so mp.spawn can pickle it."""
    ...
    torch.cuda.synchronize()
    if rank == 0:
        print(f"  NCCL all-reduce OK  sum[0]={x[0].item():.0f} "
              f"(expect {sum(range(1, world + 1))})  "
...
def nccl_allreduce():
    try:
        ...
        print("  torch.multiprocessing unavailable:", e); return
    world = torch.cuda.device_count()
    if world < 2:
        print("  <2 GPUs, skip NCCL"); return
    mp.spawn(_nccl_worker, args=(world,), nprocs=world, join=True)
